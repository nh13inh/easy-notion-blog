...
import { QueryDatabaseResponse } from '../../lib/notion/responses'
...
const ApiOgImage = async function(req: NextRequest) {
  ...
  const res: QueryDatabaseResponse = await (await fetch(`https://api.notion.com/v1/databases/${DATABASE_ID}/query`, {
    method: 'POST',
    body: JSON.stringify({
      filter: {
        and: [
  ...
  const pageObject = res.results[0]

  if (pageObject.properties.OGImage && pageObject.properties.OGImage.files.length > 0) {
    return new ImageResponse(
      (
        ...
        >
          <img src={pageObject.properties.OGImage.files[0].file.url} alt="og-image"/>
        </div>
      ),
      {
        width: 1200,
    ...
    (
      ...
      >
        ...
        >
          ...
          >
            {pageObject.properties.Page.title[0].plain_text}
          </div>
          <div
            style={{
              display: 'flex',
